...
std::map<Anaquin::SequinID, std::map<int, Anaquin::KMInfo>> __KMInfo__;

// helper functions
// pre: u is sorted
bool isUnique(const std::vector<int>& u) {
    for (auto j = 1u; j < u.size(); j++) {
        if (u[j-1] == u[j]) {
            return false;
        }
    }
...
void KmerIndex::BuildTranscripts(const ProgramOptions& opt) {
    ...
    for (auto& fasta : opt.transfasta) {
        //std::cout << "[build] loading fasta file " << fasta << std::endl;
    }
    //std::cout << "[build] k-mer length: " << k << std::endl;
    ...
    for (auto& fasta : opt.transfasta) {
        ...
        while (true) {
            ...
            if (str.size() >= 10 && str.substr(str.size()-10,10) == "AAAAAAAAAA") {
                // clip off polyA tail
                ////std::cout << "[index] clipping off polyA tail" << std::endl;
                polyAcount++;
                int j;
    ...
    if (polyAcount > 0) {
        //std::cout << "[build] warning: clipped off poly-A tail (longer than 10)" << std::endl << "        from " << polyAcount << " target sequences" << std::endl;
    }
    
    
    if (countNonNucl > 0) {
        //std::cout << "[build] warning: replaced " << countNonNucl << " non-ACGUT characters in the input sequence" << std::endl << "        with pseudorandom nucleotides" << std::endl;
    }
    if (countUNuc > 0) {
        //std::cout << "[build] warning: replaced " << countUNuc << " U characters with Ts" << std::endl;
    }
...
void KmerIndex::BuildDeBruijnGraph(const ProgramOptions& opt, const std::vector<std::string>& seqs) {
    const auto shouldKM = __KMInfo__.empty();
    
    //std::cout << "[build] counting k-mers ... "; //std::cout.flush();
    // gather all k-mers
    for (auto i = 0u; i < seqs.size(); i++) {
        ...
        auto j = 0;
        for (; kit != kit_end; ++kit) {
            if (shouldKM) { __KMInfo__[target_names_[i]][j++].kmer = kit->first.toString(); }
    ...
    //std::cout << "[build] building target de Bruijn graph ... "; //std::cout.flush();
...
void KmerIndex::BuildEquivalenceClasses(const ProgramOptions& opt, const std::vector<std::string>& seqs) {
    //std::cout << "[build] creating equivalence classes ... "; //std::cout.flush();
    ...
    int perftr = 0;
    for (auto i = 0u; i < trinfos.size(); i++) {
        bool all = true;
...
void KmerIndex::FixSplitContigs(const ProgramOptions& opt, std::vector<std::vector<TRInfo>>& trinfos) {
    ...
    for (int i = 0; i < orig_size; i++) {
        ...
        if (all) {
            ...
            for (auto j = 1u; j < brpoints.size(); j++) {
...
void KmerIndex::write(const std::string& index_out, bool writeKmerTable) {
    ...
    if (!out.is_open()) {
        ...
        //std::cout << "Error: index output file could not be opened!";
        exit(1);
    }
...
void KmerIndex::load(ProgramOptions& opt, bool loadKmerTable) {
    ...
    if (!in.is_open()) {
        // TODO: better handling
        //std::cout << "Error: index input file could not be opened!";
        exit(1);
    }
    ...
    if (header_version != INDEX_VERSION) {
        //std::cout << "Error: incompatible indices. Found version " << header_version << ", expected version " << INDEX_VERSION << std::endl
    ...
    if (Kmer::k == 0) {
        ////std::cout << "[index] no k has been set, setting k = " << k << std::endl;
        Kmer::set_k(k);
        opt.k = k;
    } else if (Kmer::k == k) {
        ////std::cout << "[index] Kmer::k has been set and matches" << k << std::endl;
        opt.k = k;
    } else {
        //std::cout << "Error: Kmer::k was already set to = " << Kmer::k << std::endl
...
//    //std::cout << "[index] k-mer length: " << k << std::endl;
//    //std::cout << "[index] number of targets: " << pretty_num(num_trans)
//    << std::endl;
//    //std::cout << "[index] number of k-mers: " << pretty_num(kmap_size)
//    << std::endl;
...
//    //std::cout << "[index] number of equivalence classes: "
...
int KmerIndex::mapPair(const char *s1, int, const char *s2, int, int) const {
    bool d1 = true;
    bool d2 = true;
    int p1 = -1;
    int p2 = -1;
    ...
    if ((d1 && d2) || (!d1 && !d2)) {
        ////std::cout << "Reads map to same strand " << s1 << "\t" << s2 << std::endl;
        return -1;
    }
